...
This repository contains MicroK8s addons used to provide configuration for The University of Sydney.

  * nat64, IPv6 to IPv4 Network Address Translation with CoreDNS dns64 plugin enabled. Support IPv6 cluster in IPv4 networks.

### How to use an addons repository

#### Adding repositories
3rd party addons repositories are supported on MicroK8s v1.24 and onwards. To add a repository on an already installed MicroK8s you have to use the `microk8s addons repo` command and provide a user friendly repo name, the path to the repository and optionally a branch within the repository. For example, to add this repository:

```
microk8s addons repo add usyd https://github.com/SydneyResearchTech/microk8s-addons --reference main
```
...
```
microk8s addons repo add usyd ./microk8s-addons
```
